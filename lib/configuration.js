...
class ElasticsearchConfiguration {
  ...
  constructor(options=constants.ELASTIC) {
    if (options.url) {
      console.warn('options.url is deprecated, use options.hosts array instead');
    }
    this.hosts = options.hosts || [options.url ? options.url : `http://${options.host}:${options.port}`];
  ...
  _getIndexConfiguration(type) {
    return this._getElasticsearchVersion().then(version => {
      const configurationPath = this.path || path.join(__dirname, `../data/${version}.x/`);
   ...
   */
  _getElasticsearchVersion() {
    return this._getClient()
      .then(esClient => q.ninvoke(esClient, 'info'))
      .then(([{ version }, statusCode]) => version.number.split('.', 1)[0]);
  }
  ...
  _getClient() {
    if (!this.client) {
      this.client = new elasticsearch.Client(Object.assign({}, { hosts: this.hosts }, this.httpAuth ? { httpAuth: this.httpAuth } : {}));
    }
